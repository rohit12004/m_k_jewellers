    ...
    switch (error.code) {
        ...
        default:
            ...
            if (
                ...
                return {
                    statusCode: 503,
                    message: 'Database connection failed. The server is unable to reach the database.'
                };
            }
            return null;
    }
...
export const catchError = (error, customMessage) => {
    ...
    // If no custom message, use a generic fallback instead of a raw technical error
    if (!customMessage && error.message && !error.message.includes('Prisma') && !error.message.includes('database')) {
