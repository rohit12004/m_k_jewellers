    ...
    if (media?.length) {
      ...
      for (let i = 0; i < media.length; i++) {
        await prisma.media.update({
          where: { id: media[i] },
          data: { productId: newProduct.id, order: i },
        });
      }
    }
...
export async function updateProduct(id, data) {
  try {
    ...
    if (data.media?.length) {
      // Reset previous relations for this product
      await prisma.media.updateMany({
        where: { productId: id },
        data: { productId: null },
      });

      // ✅ Update each media with the new product relation and sequence order
      for (let i = 0; i < data.media.length; i++) {
        await prisma.media.update({
          where: { id: data.media[i] },
          data: { productId: id, order: i },
        });
      }
    }
