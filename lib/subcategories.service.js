  ...
  try {
    ...
    const newSubCategory = await prisma.subCategory.create({
      data: {
        ...
        slug,
        category: {
          connect: { id: categoryId }, // Connect sub-category to parent category
        },
      },
    })

    return response(true, 201, 'Sub-category created successfully.', newSubCategory)
  } catch (error) {
    if (error.code === 'P2002') {
...
export async function updateSubCategory(id, data) {
  try {
    const { name, slug, categoryId } = data; // include categoryId only if needed
    ...
    const updatedSubCategory = await prisma.subCategory.update({
      ...
      data: {
        name,
        slug,
        ...(categoryId && { categoryId }), // optional category update
        updatedAt: new Date(),
      },
    });

    return response(true, 200, "Subcategory updated successfully.", updatedSubCategory);
  } catch (error) {
...
export async function deleteManySubCategory(ids) {
  try {
    ...
    throw new Error("Failed to permanently delete subcategory")
  }
}
