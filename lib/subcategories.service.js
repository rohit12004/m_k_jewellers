import prisma from "../lib/prisma.js";
import { response, catchError } from "./helperFunction.js";
import { validate as isValidUUID } from "uuid";


/**
 * Normalizes subcategory data:
 * - Name: Singular + Title Case (e.g., "rings" -> "Ring")
 * - Slug: Singular + lowercase (e.g., "RINGS" -> "ring")
 */
function normalizeSubCategoryData(data) {
  const { name, slug } = data;

  const singularize = (str) => {
    if (!str) return str;
    const lower = str.toLowerCase().trim();
    if (lower.endsWith('ies')) return lower.slice(0, -3) + 'y';
    if (lower.endsWith('s') && !lower.endsWith('ss')) return lower.slice(0, -1);
    return lower;
  };

  const toTitleCase = (str) => {
    if (!str) return str;
    return str.charAt(0).toUpperCase() + str.slice(1).toLowerCase();
  };

  const normalizedSlug = singularize(slug || name);
  const normalizedName = toTitleCase(singularize(name));

  return {
    ...data,
    name: normalizedName,
    slug: normalizedSlug
  };
}

export async function createSubCategory(data) {
  try {
    const { name, slug, categoryIds, mediaId } = normalizeSubCategoryData(data)

    // Create subcategory first
    const newSubCategory = await prisma.subCategory.create({
      data: {
        name,
        slug,
      },
    })

    // Create junction table entries for each category
    if (categoryIds && categoryIds.length > 0) {
      await prisma.categorySubCategory.createMany({
        data: categoryIds.map(categoryId => ({
          categoryId,
          subCategoryId: newSubCategory.id,
        })),
      })
    }

    // Link media if provided
    if (mediaId) {
      try {
        await prisma.media.update({
          where: { id: mediaId },
          data: { subCategoryId: newSubCategory.id },
        })
      } catch (mediaError) {
      }
    } else {
    }

    // Fetch the subcategory with categories and media to return
    const subcategoryWithCategories = await prisma.subCategory.findUnique({
      where: { id: newSubCategory.id },
      include: {
        categorySubCategories: {
          include: {
            category: {
              select: {
                id: true,
                name: true,
                slug: true,
              }
            }
          }
        },
        media: {
          select: {
            id: true,
            secure_url: true,
            alt: true,
            title: true,
          },
        },
      }
    })

    console.log('Final subcategory with media:', JSON.stringify(subcategoryWithCategories.media))

    return response(true, 201, 'Sub-category created successfully.', subcategoryWithCategories)
  } catch (error) {
    return catchError(error)
  }
}


export async function getFilteredSubCategories({ matchQuery, orderBy, start, size }) {
  const [data, total] = await Promise.all([
    prisma.subCategory.findMany({
      where: matchQuery,
      orderBy: Object.keys(orderBy).length ? orderBy : { createdAt: "desc" },
      skip: start,
      take: size,
      select: {
        id: true,
        name: true,
        slug: true,
        categorySubCategories: {
          include: {
            category: {
              select: {
                id: true,
                name: true,
              }
            }
          }
        },
        createdAt: true,
        updatedAt: true,
        deletedAt: true,
      },
    }),
    prisma.subCategory.count({ where: matchQuery }),
  ])

  return { data: data || [], total: total || 0 }
}


export async function getSingleSubCategory(id) {
  try {
    // ✅ Validate ID format (UUID)
    if (!isValidUUID(id)) {
      return response(false, 400, "Invalid UUID.");
    }

    // ✅ Fetch single subcategory
    const subcategory = await prisma.subCategory.findFirst({
      where: {
        id,
        deletedAt: null, // only fetch non-deleted
      },
      include: {
        categorySubCategories: {
          include: {
            category: {
              select: {
                id: true,
                name: true,
                slug: true
              }
            }
          }
        },
        media: {
          select: {
            id: true,
            secure_url: true,
            alt: true,
            title: true,
          },
        },
      }
    });

    if (!subcategory) {
      return response(false, 404, "Subcategory not found.");
    }

    return response(true, 200, "Subcategory fetched successfully", subcategory);

  } catch (error) {
    return catchError(error, "Failed to fetch subcategory");
  }
}

export async function updateSubCategory(id, data) {
  try {
    const { name, slug, categoryIds, mediaId } = normalizeSubCategoryData(data);

    // ✅ Update subcategory record
    const updatedSubCategory = await prisma.subCategory.update({
      where: { id },
      data: {
        name,
        slug,
        updatedAt: new Date(),
      },
    });

    // ✅ Update category relationships if categoryIds provided
    if (categoryIds !== undefined) {
      // Delete existing relationships
      await prisma.categorySubCategory.deleteMany({
        where: { subCategoryId: id },
      });

      // Create new relationships
      if (categoryIds.length > 0) {
        await prisma.categorySubCategory.createMany({
          data: categoryIds.map(categoryId => ({
            categoryId,
            subCategoryId: id,
          })),
        });
      }
    }

    // ✅ Update media if provided
    if (mediaId !== undefined) {
      // First, unlink any existing media
      await prisma.media.updateMany({
        where: { subCategoryId: id },
        data: { subCategoryId: null },
      });

      // Then link the new media if provided
      if (mediaId) {
        await prisma.media.update({
          where: { id: mediaId },
          data: { subCategoryId: id },
        });
      }
    }

    // Fetch updated subcategory with categories and media
    const subcategoryWithCategories = await prisma.subCategory.findUnique({
      where: { id },
      include: {
        categorySubCategories: {
          include: {
            category: {
              select: {
                id: true,
                name: true,
                slug: true,
              }
            }
          }
        },
        media: {
          select: {
            id: true,
            secure_url: true,
            alt: true,
            title: true,
          },
        },
      }
    });

    return response(true, 200, "Subcategory updated successfully.", subcategoryWithCategories);
  } catch (error) {
    return catchError(error, "Failed to update subcategory.");
  }
}


export async function getSubCategoryById(ids) {
  try {
    const subcategories = await prisma.subCategory.findMany({
      where: { id: { in: ids } },
      select: {
        id: true,
        name: true,
        slug: true,
        categorySubCategories: {
          include: {
            category: {
              select: { id: true, name: true }
            }
          }
        },
        createdAt: true,
        updatedAt: true,
      },
    })

    return subcategories
  } catch (error) {
    throw error
  }
}


export async function softDeleteSubCategory(ids) {
  try {
    const result = await prisma.subCategory.updateMany({
      where: {
        id: { in: ids },
      },
      data: {
        deletedAt: new Date(),
      },
    })

    return result
  } catch (error) {
    throw error
  }
}

export async function restoreSubCategory(ids) {
  try {
    const result = await prisma.subCategory.updateMany({
      where: {
        id: { in: ids },
      },
      data: {
        deletedAt: null,
      },
    })

    return result
  } catch (error) {
    throw new Error("Failed to restore subcategory")
  }
}

export async function deleteManySubCategory(ids) {
  try {
    // First, check if any products are using these subcategories
    const productsCount = await prisma.product.count({
      where: {
        subCategoryId: { in: ids },
        deletedAt: null, // Only count non-deleted products
      },
    })

    if (productsCount > 0) {
      return {
        success: false,
        statusCode: 400,
        message: `Cannot delete subcategory. ${productsCount} product${productsCount > 1 ? 's are' : ' is'} associated with this subcategory. Please remove or reassign the product${productsCount > 1 ? 's' : ''} first.`,
      }
    }

    // Delete all junction table entries for these subcategories
    await prisma.categorySubCategory.deleteMany({
      where: {
        subCategoryId: { in: ids },
      },
    })

    // Now delete the subcategories
    const result = await prisma.subCategory.deleteMany({
      where: {
        id: { in: ids },
      },
    })

    return {
      success: true,
      statusCode: 200,
      message: "Subcategory deleted successfully",
      data: result,
    }
  } catch (error) {
    return {
      success: false,
      statusCode: (error.statusCode || 500),
      message: error.message || "Failed to delete subcategory",
    }
  }
}


export async function getSubCategoriesForExport({ filter = { deletedAt: null }, orderBy = { createdAt: 'desc' } } = {}) {
  try {
    const subcategories = await prisma.subCategory.findMany({
      where: filter,
      orderBy,
      select: {
        id: true,
        name: true,
        slug: true,
        categorySubCategories: {
          include: {
            category: {
              select: { id: true, name: true }
            }
          }
        },
        createdAt: true,
        updatedAt: true,
        deletedAt: true,
      },
    });
    return subcategories;
  } catch (error) {
    throw error;
  }
}
