...
Returns a list of actual products with name, price, category, images, and links.`,
    ...
    func: async ({ category, subCategory, minPrice, maxPrice, purity, gender, keywords, limit = 5 }) => {
        try {
            ...
            // SubCategory filter logic with robustness
            if (subCategory) {
                ...
                const subCategorySingular = singularize(subCategory);
                
                console.log(`🔍 [ProductSearch] Normalized subCategory: "${subCategory}" -> "${subCategorySingular}"`);

                whereClause.AND.push({
                    OR: [
                        { subCategory: { slug: subCategorySingular } },
                        { subCategory: { name: { equals: subCategorySingular, mode: 'insensitive' } } },
                        { subCategory: { name: { equals: subCategory, mode: 'insensitive' } } } // Original too
                    ]
                });
            }
