...
Returns complete product information including all variants, images, and specifications.`,
    ...
    func: async ({ identifier }) => {
        try {
            ...
            // Fetch metal rates for accurate pricing
            const metalRates = await prisma.metalRate.findMany();

            // Calculate prices for each variant using the same logic as productSearch
            const { calculatePrice } = await import("@/lib/pricing/calculatePrice");

            const variantsWithPrices = product.variants.map(variant => {
                const priceDetails = calculatePrice(product, variant, metalRates);
                ...
                return {
                    ...
                    basePrice: priceDetails.subtotal,
                    finalPrice: priceDetails.finalPrice,
                    priceBreakdown: {
                        metalCost: priceDetails.metalCost,
                        labourCharge: priceDetails.labourCharge,
                        hallmarkCharges: priceDetails.hallmarkCharges,
