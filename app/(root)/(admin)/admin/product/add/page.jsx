...
const formSchema = z.object({
  ...
  variants: z.array(z.object({
    ...
    size: z.string().optional(), // Optional - for rings only
    length: z.string().optional(), // Optional - for chains/mangalsutra only
...
const AddProduct = () => {
  ...
  useEffect(() => {
    if (fetchSubCategory?.success && selectedCategoryId) {
      ...
      const filtered = fetchSubCategory.data.filter(sc =>
        sc.categorySubCategories?.some(csc => csc.category.id === selectedCategoryId)
      )
                  ...
                  <div className='flex gap-4 justify-center my-3 flex-wrap'>
                    {selectedMedia?.map((media, index) => (
                      <div key={media.id} className="relative group">
                        <Image src={media.url} height={80} width={80} className='object-cover rounded border h-20 w-20' alt="" />
